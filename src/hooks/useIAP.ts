...
const waitForPurchaseGlobal = async () => {
  ...
  if (!window.CdvPurchase?.store) {
    throw new Error(
      "Apple purchases are not available in this iOS build. Run npm install, then npx cap sync ios.",
    );
  }
...
const initializePurchaseStore = async (validateReceipt: ReceiptValidator) => {
  ...
  storeInitPromise = (async () => {
    ...
    const errors = await store.initialize([
      { platform, options: { needAppReceipt: false, autoFinish: false } },
    ]);
...
/**
 * Native Apple In-App Purchases hook using cordova-plugin-purchase (StoreKit bridge).
  ...
  const loadProducts = useCallback(async () => {
    ...
    try {
      ...
      const mapped: IAPProduct[] = IAP_PRODUCT_IDS.map((id) =>
        store.get(id, cdv.Platform.APPLE_APPSTORE),
      )
        .filter(Boolean)
        .map((p: any) => {
          ...
          return {
            ...
            price:
              typeof pricing?.priceMicros === "number" ? pricing.priceMicros / 1_000_000 : 0,
  ...
  const purchase = useCallback(
    async (productId: IAPProductId) => {
      ...
      try {
        ...
        if (!offer)
          throw new Error("This Apple product is not available yet. Check App Store Connect and try again.");
